...
const ThreadDetail = (): JSX.Element => {
  ...
  const { data, isSuccess, isLoading, isError } = useGetThreadByIdQuery(threadId ?? skipToken);
    ...
    <div className="my-12 space-y-8">
      ...
      {isSuccess && (
        <div className="mb-8">
          ...
          <span className="rounded-full p-1 px-2 text-sm text-zinc-600 ring-1 ring-violet-400">{data?.category}</span>
        </div>
      )}
